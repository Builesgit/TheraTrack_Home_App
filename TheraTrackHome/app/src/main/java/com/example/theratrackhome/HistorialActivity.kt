...
class HistorialActivity : AppCompatActivity() {
    ...
    private fun enlazarVistas() {
        layoutMediciones = findViewById(R.id.layoutMediciones)
        tvSinRegistros   = findViewById(R.id.tvSinRegistros)
        lineChart        = findViewById(R.id.lineChart)
        chipHoy          = findViewById(R.id.chipHoy)
        chip7Dias        = findViewById(R.id.chip7Dias)
        chip30Dias       = findViewById(R.id.chip30Dias)
    }

    private fun configurarBottomNav() {
        val bottomNav = findViewById<BottomNavigationView>(R.id.bottomNavigation)
        bottomNav.menu.findItem(R.id.nav_historial)?.isChecked = true
    ...
    private fun configurarChips() {
        fun activarChip(activo: TextView, vararg inactivos: TextView) {
            ...
            inactivos.forEach {
                ...
                it.setTextColor(getColor(R.color.colorTextSecondary))
    ...
    private fun mostrarLista(registros: List<RegistroDiario>) {
        ...
        registros.reversed().forEach { registro ->
            val item = LayoutInflater.from(this).inflate(R.layout.item_medicion, layoutMediciones, false)
